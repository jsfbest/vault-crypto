...
```
src/
├── main.ts              # 插件入口（890+ 行）：生命周期 + 命令 + 右键菜单 + 事件 + 三态管理 + 自动锁定 + 导出加密HTML
...
- 插件命令"导出加密HTML"：生成嵌入式 HTML（密文内嵌，文件名匹配原 .md），桌面端调系统保存对话框，移动端存到 vault 原文件目录
...
- 右键菜单：加密文件 / 查看加密文件 / 完全解密文件 / 加密文件夹 / 完全解密文件夹 / 导出加密HTML
- 编辑区右键：查看加密文件
- 命令面板：加密当前文件 / 查看加密当前文件 / 完全解密当前文件 / 加密文件夹 / 完全解密文件夹 / 导出加密HTML
